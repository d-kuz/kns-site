...
import com.pks.services.UserService;
...
public class UserController {
    ...
    public String saveUserChangeRole(@RequestParam(name = "key") String key, Principal principal, Model model) {
        User user = userService.getUserByPrincipal(principal);
        if(key.equals("2110")){
            userService.changeUserRoles(user);
        }else {
            model.addAttribute("errorMessage", "Не правильный код!");
        }
        return "userChange";
    }
}
